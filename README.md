...
The initial database is created using numpy, citipy module and openweather API. Random pairs of latitues and longitudes were generated for northern and southern hemispheres using np.random.uniform() function. Then nearby city search is performed using CityPy module in Python. At the same time, the corresponding weather data for the cities is scraped using Python requests & OpenWeather API. This database is extracted and stored as a csv file for further use.
...
In this step, the csv file stored in the earlier step is imported into the python environment and loaded into a dataframe. The user input is taken for user's weather preference for the vacation destinations. As per the user preference, the dataframe is then filtered to have the only destinations that satisfy the user criteria. Using nearby search, a hotel in each city is added to the our dataframe and then the dataframe is cleaned to show the valid and complete data. Using google maps APIs the vacation search results are displayed in the form of maps and location markers for destination cities. 
